...
class UserData:
    ...
    def set_top_genres(self, artists):
        ...
        self.top_genres = {
            k: v
            for k, v in sorted(
                self.top_genres.items(), key=lambda item: item[1], reverse=True
            )
        }
    ...
    def compare_with(self, other):
        ...
        shared_artists = [
            artist for artist in self.top_artists if artist in other.top_artists
        ]
        # find any shared genres between this and other
        shared_genres = [
            genre for genre in self.top_genres if genre in other.top_genres
        ][:50]
        # find any shared tracks between this and other
        shared_tracks = [
            track for track in self.top_tracks if track in other.top_tracks
        ]
        match_percentage = 100 * (
            (len(shared_artists) + len(shared_genres) + len(shared_tracks)) / 150
        )
        return {
    ...
    def to_dict(self):
        return {"name": self.name, "country": self.country, "pfp_url": self.pfp_url}
